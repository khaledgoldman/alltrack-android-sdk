...
public final class AlltrackLinkResolution {
    public interface AlltrackLinkResolutionCallback {
    ...
    private AlltrackLinkResolution() { }

    public static void resolveLink(final String url,
                                   final String[] resolveUrlSuffixArray,
                                   final AlltrackLinkResolutionCallback alltrackLinkResolutionCallback)
    {
        if (alltrackLinkResolutionCallback == null) {
            return;
        }

        if (url == null) {
            alltrackLinkResolutionCallback.resolvedLinkCallback(null);
            return;
        }
        ...
        if (! urlMatchesSuffix(originalURL.getHost(), resolveUrlSuffixArray)) {
            alltrackLinkResolutionCallback.resolvedLinkCallback(
                    AlltrackLinkResolution.convertToUri(originalURL));
            return;
        }
        ...
        executor.execute(new Runnable() {
            @Override
            public void run() {
                requestAndResolve(finalOriginalURL, 0, alltrackLinkResolutionCallback);
            }
        });
    }
            ...
            final AlltrackLinkResolutionCallback alltrackLinkResolutionCallback)
    {
        ...
        if (responseUrl == null) {
            alltrackLinkResolutionCallback.resolvedLinkCallback(
                    AlltrackLinkResolution.convertToUri(previousUrl));
            return;
        }
        ...
        if (isTerminalUrl(responseUrl.getHost())) {
            alltrackLinkResolutionCallback.resolvedLinkCallback(
                    AlltrackLinkResolution.convertToUri(responseUrl));
            return;
        }
        ...
        requestAndResolve(responseUrl, recursionNumber, alltrackLinkResolutionCallback);
    }
                                          ...
                                          final AlltrackLinkResolutionCallback alltrackLinkResolutionCallback)
    {
        ...
        try {
            ...
            resolveLink(resolvedURL,
                    httpsUrl,
                    recursionNumber + 1,
                    alltrackLinkResolutionCallback);
        }
    }
