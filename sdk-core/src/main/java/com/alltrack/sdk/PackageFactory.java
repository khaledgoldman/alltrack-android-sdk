...
import static com.alltrack.sdk.Constants.MALFORMED;
...
public class PackageFactory {
    ...
    public static ActivityPackage buildReftagSdkClickPackage(final String rawReferrer,
        ...
        try {
            ...
            referrer = MALFORMED;
            AlltrackFactory.getLogger().error("Referrer decoding failed due to UnsupportedEncodingException. Message: (%s)", e.getMessage());
        } catch (IllegalArgumentException e) {
            referrer = MALFORMED;
            AlltrackFactory.getLogger().error("Referrer decoding failed due to IllegalArgumentException. Message: (%s)", e.getMessage());
        } catch (Exception e) {
            referrer = MALFORMED;
            AlltrackFactory.getLogger().error("Referrer decoding failed. Message: (%s)", e.getMessage());
        }

        AlltrackFactory.getLogger().verbose("Referrer to parse (%s)", referrer);
    ...
    public static ActivityPackage buildDeeplinkSdkClickPackage(final Uri url,
        ...
        try {
            ...
            AlltrackFactory.getLogger().error("Deeplink url decoding failed due to UnsupportedEncodingException. Message: (%s)", e.getMessage());
        } catch (IllegalArgumentException e) {
            urlStringDecoded = urlString;
            AlltrackFactory.getLogger().error("Deeplink url decoding failed due to IllegalArgumentException. Message: (%s)", e.getMessage());
        } catch (Exception e) {
            urlStringDecoded = urlString;
            AlltrackFactory.getLogger().error("Deeplink url decoding failed. Message: (%s)", e.getMessage());
        }

        AlltrackFactory.getLogger().verbose("Url to parse (%s)", urlStringDecoded);
    ...
    private static PackageBuilder queryStringClickPackageBuilder(
        ...
        AlltrackAttribution queryStringAttribution = new AlltrackAttribution();
    ...
    private static boolean readQueryString(final String key,
                                           ...
                                           AlltrackAttribution queryStringAttribution) {
        if (key == null || value == null) {
            return false;
        }

        // Parameter key does not start with "alltrack_" prefix.
        if (!key.startsWith(ALLTRACK_PREFIX)) {
            return false;
        }

        String keyWOutPrefix = key.substring(ALLTRACK_PREFIX.length());
    ...
    private static boolean tryToSetAttribution(AlltrackAttribution queryStringAttribution,
                                               final String key,
                                               final String value) {
