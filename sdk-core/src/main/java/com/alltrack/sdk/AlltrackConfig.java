...
public class AlltrackConfig {
    ...
    Double delayStart;
    AlltrackInstance.PreLaunchActions preLaunchActions;
    ILogger logger;
    String userAgent;
    String pushToken;
    ...
    public AlltrackConfig(Context context, String appToken, String environment) {
        init(context, appToken, environment, false);
    }

    public AlltrackConfig(Context context, String appToken, String environment, boolean allowSuppressLogLevel) {
    ...
    private void init(Context context, String appToken, String environment, boolean allowSuppressLogLevel) {
        ...
        // default values
        if (allowSuppressLogLevel && AlltrackConfig.ENVIRONMENT_PRODUCTION.equals(environment)) {
    ...
    private void setLogLevel(LogLevel logLevel, String environment) {
        logger.setLogLevel(logLevel, AlltrackConfig.ENVIRONMENT_PRODUCTION.equals(environment));
    }
    ...
    private boolean checkEnvironment(String environment) {
        ...
        if (environment.equals(AlltrackConfig.ENVIRONMENT_SANDBOX)) {
            logger.warnInProduction("SANDBOX: Alltrack is running in Sandbox mode. " +
                    "Use this setting for testing. " +
                    "Don't forget to set the environment to `production` before publishing!");
            return true;
        }
        if (environment.equals(AlltrackConfig.ENVIRONMENT_PRODUCTION)) {
            logger.warnInProduction(
                    "PRODUCTION: Alltrack is running in Production mode. " +
