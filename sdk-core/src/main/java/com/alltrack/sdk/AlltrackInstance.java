...
/**
 * Class used to forward instructions to SDK which user gives as part of Alltrack class interface.
 *
 * @author Pedro Silva (@nonelse)
 * @since 12th April 2014
 */
public class AlltrackInstance {
    public static class PreLaunchActions {
        public List<IRunActivityHandler> preLaunchActionsArray;
        public List<AlltrackThirdPartySharing> preLaunchAlltrackThirdPartySharingArray;
        public Boolean lastMeasurementConsentTracked;

        public PreLaunchActions() {
            preLaunchActionsArray = new ArrayList<>();
            preLaunchAlltrackThirdPartySharingArray = new ArrayList<>();
    ...
    /**
     * Base path for Alltrack packages.
     */
     ...
     */
    public void onCreate(final AlltrackConfig alltrackConfig) {
        ...
        if (activityHandler != null) {
            AlltrackFactory.getLogger().error("Alltrack already initialized");
            return;
        }

        alltrackConfig.preLaunchActions = preLaunchActions;
        alltrackConfig.pushToken = pushToken;
        alltrackConfig.startEnabled = startEnabled;
        alltrackConfig.startOffline = startOffline;
        alltrackConfig.basePath = this.basePath;
        alltrackConfig.gdprPath = this.gdprPath;
        alltrackConfig.subscriptionPath = this.subscriptionPath;

        activityHandler = AlltrackFactory.getActivityHandler(alltrackConfig);
        setSendingReferrersAsNotSent(alltrackConfig.context);
    }
    ...
    public void appWillOpenUrl(final Uri url, final Context context) {
        ...
        if (url == null || url.toString().length() == 0) {
            AlltrackFactory.getLogger().warn(
     ...
     * Used only for Alltrack tests, shouldn't be used in client apps.
     */
    ...
    public void trackThirdPartySharing(final AlltrackThirdPartySharing alltrackThirdPartySharing) {
        if (!checkActivityHandler("third party sharing", true)) {
            preLaunchActions.preLaunchAlltrackThirdPartySharingArray.add(alltrackThirdPartySharing);
            return;
        }

        activityHandler.trackThirdPartySharing(alltrackThirdPartySharing);
    }
    ...
    public void trackAdRevenue(final AlltrackAdRevenue alltrackAdRevenue) {
        ...
        activityHandler.trackAdRevenue(alltrackAdRevenue);
    }
     ...
     */
    public void trackPlayStoreSubscription(AlltrackPlayStoreSubscription subscription) {
    ...
    private boolean checkActivityHandler(final String action, final boolean actionSaved) {
        ...
        if (action == null) {
            AlltrackFactory.getLogger().error("Alltrack not initialized correctly");
            return false;
        }

        if (actionSaved) {
            AlltrackFactory.getLogger().warn(
                    "Alltrack not initialized, but %s saved for launch",
                    action);
        } else {
            AlltrackFactory.getLogger().warn(
                    "Alltrack not initialized, can't perform %s",
                    action);
        }
        return false;
    }
    ...
    /**
     * Check if AlltrackInstance enable flag is set or not.
     *
     * @return boolean indicating whether AlltrackInstance is enabled or not
     */
     ...
     */
    public void setTestOptions(AlltrackTestOptions testOptions) {
        ...
        if (testOptions.baseUrl != null) {
            AlltrackFactory.setBaseUrl(testOptions.baseUrl);
        }
        if (testOptions.gdprUrl != null) {
            AlltrackFactory.setGdprUrl(testOptions.gdprUrl);
        }
        if (testOptions.subscriptionUrl != null) {
            AlltrackFactory.setSubscriptionUrl(testOptions.subscriptionUrl);
        }
        if (testOptions.timerIntervalInMilliseconds != null) {
            AlltrackFactory.setTimerInterval(testOptions.timerIntervalInMilliseconds);
        }
        if (testOptions.timerStartInMilliseconds != null) {
            AlltrackFactory.setTimerStart(testOptions.timerIntervalInMilliseconds);
        }
        if (testOptions.sessionIntervalInMilliseconds != null) {
            AlltrackFactory.setSessionInterval(testOptions.sessionIntervalInMilliseconds);
        }
        if (testOptions.subsessionIntervalInMilliseconds != null) {
            AlltrackFactory.setSubsessionInterval(testOptions.subsessionIntervalInMilliseconds);
        }
        if (testOptions.tryInstallReferrer != null) {
            AlltrackFactory.setTryInstallReferrer(testOptions.tryInstallReferrer);
        }
        if (testOptions.noBackoffWait != null) {
            AlltrackFactory.setPackageHandlerBackoffStrategy(BackoffStrategy.NO_WAIT);
            AlltrackFactory.setSdkClickBackoffStrategy(BackoffStrategy.NO_WAIT);
        }
        if (testOptions.enableSigning != null && testOptions.enableSigning) {
            AlltrackFactory.enableSigning();
        }
        if (testOptions.disableSigning != null && testOptions.disableSigning) {
            AlltrackFactory.disableSigning();
        }
    }
}
