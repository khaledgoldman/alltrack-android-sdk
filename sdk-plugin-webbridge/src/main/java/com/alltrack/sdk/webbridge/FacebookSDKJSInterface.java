...
public class FacebookSDKJSInterface {
    ...
    public static String getApplicationId(Context context) {
        ...
        try {
            ...
            AlltrackFactory.getLogger().error("Error loading fb ApplicationInfo: %s", e.getMessage());
            return null;
        }
        ...
        if (appId instanceof String) {
            ...
            AlltrackFactory.getLogger().error("App Ids cannot be directly placed in the manifest." +
                    "They must be prefixed by 'fb' or be placed in the string resource file.");
        } else {
            AlltrackFactory.getLogger().error("App Ids is not a string or integer");
        }
    ...
    public void sendEvent(String pixelId, String event_name, String jsonString) {
        if (pixelId == null) {
            AlltrackFactory.getLogger().error("Can't bridge an event without a referral Pixel ID. " +
        ...
        if (eventToken == null) {
            AlltrackFactory.getLogger().debug("No mapping found for the fb pixel event %s, trying to fall back to the default event token", event_name);
            eventToken = this.fbPixelDefaultEventToken;
        }

        if (eventToken == null) {
            AlltrackFactory.getLogger().warn("There is not a default event token configured or a mapping found for event named: '%s'. It won't be tracked as an alltrack event", event_name);
            return;
        }
        AlltrackEvent fbPixelEvent = new AlltrackEvent(eventToken);
        ...
        AlltrackFactory.getLogger().debug("FB pixel event received, eventName: %s, payload: %s", event_name, stringMap);
